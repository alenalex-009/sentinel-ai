...
const TOTAL_CAPACITY = DEMO_CANDIDATE_SITES.reduce((s, site) => s + site.safe_capacity, 0)
...
function ImpactMetric({
  label, baseline, simulated, unit = '', higherIsBad = true,
}: {
  label: string
  baseline: number
  simulated: number
  ...
  return (
    <div className="rounded border border-slate-800 bg-slate-900 p-2.5">
      ...
      <div className="text-2xs text-slate-600">Baseline: {baseline.toLocaleString()}{unit}</div>
    </div>
  )
}
...
export function ScenarioAnalysis() {
  ...
  const comparisonData = [
    { name: 'Risk', baseline: BASELINE_RISK, simulated: result.simulated_risk },
    { name: 'Demand', baseline: BASELINE_DEMAND / 100, simulated: result.simulated_demand / 100 },
    { name: 'Capacity', baseline: TOTAL_CAPACITY / 100, simulated: result.simulated_capacity_available / 100 },
  ]

  const gapData = [
    { name: 'Demand', value: result.simulated_demand, fill: '#ef4444' },
    { name: 'Capacity', value: result.simulated_capacity_available, fill: '#14b8a6' },
  ]
  ...
  return (
    <div className="flex h-full overflow-hidden">
      ...
      <div className="flex w-72 flex-shrink-0 flex-col gap-4 overflow-y-auto border-r border-slate-800 bg-slate-950 p-4">
        {/* SIMULATION warning */}
        <div className="rounded border border-cyan-500/30 bg-cyan-500/8 px-3 py-2">
          ...
          <p className="text-2xs text-cyan-400/70 leading-relaxed">
            All results are SIMULATED. Not live data. For planning and demonstration purposes only.
          </p>
        </div>

        {/* Source habitation */}
        <div className="rounded-lg border border-slate-800 bg-slate-900 p-3">
          <div className="text-2xs text-slate-500 mb-1">Scenario Habitation</div>
        ...
        {/* Presets */}
        <div>
          <div className="text-xs font-semibold text-slate-500 uppercase tracking-wide mb-2">Presets</div>
          <div className="flex flex-col gap-1.5">
        ...
        {/* Manual controls */}
        <div>
          <div className="text-xs font-semibold text-slate-500 uppercase tracking-wide mb-3">Manual Controls</div>
          <div className="flex flex-col gap-4">
            <SliderRow
              label="Rainfall multiplier"
              value={params.rainfall_multiplier}
              min={0.5} max={3.0} step={0.1} unit="×"
              onChange={v => update({ rainfall_multiplier: v, label: 'Custom' })}
            />
            <SliderRow
              label="Population change"
              value={params.population_change_pct}
              min={-20} max={50} step={5} unit="%"
              onChange={v => update({ population_change_pct: v, label: 'Custom' })}
            />
            <SliderRow
              label="Capacity reduction"
              value={params.capacity_reduction_pct}
              min={0} max={60} step={5} unit="%"
              onChange={v => update({ capacity_reduction_pct: v, label: 'Custom' })}
            />

            {/* Road disruption toggle */}
            <div className="flex items-center justify-between">
              <span className="text-xs text-slate-400">Road disruption (Site B)</span>
              <button
                onClick={() => update({ road_disruption: !params.road_disruption, label: 'Custom' })}
                className={clsx(
              ...
              >
                <span className={clsx(
                  ...
                  params.road_disruption ? 'translate-x-4' : 'translate-x-0.5'
                )} />
              </button>
            </div>
          </div>
      ...
      <div className="flex flex-1 flex-col overflow-y-auto gap-4 p-4">
        {/* SIMULATED banner */}
        <div className="rounded border border-cyan-500/40 bg-cyan-500/8 px-4 py-3">
          <div className="flex items-start justify-between">
            <div>
              <div className="flex items-center gap-2 mb-1">
                <DataTypeBadge type="SIMULATED" />
                <span className="text-xs font-semibold text-cyan-300">{params.label}</span>
              </div>
              <p className="text-xs text-cyan-400/80 leading-relaxed">{result.warning}</p>
            </div>
            <FreshnessBadge status="SIMULATION" />
          </div>
          {result.impact_summary && (
            <div className="mt-2 rounded border border-cyan-500/20 bg-cyan-500/5 px-2.5 py-1.5">
              <p className="text-xs text-cyan-300 font-mono">{result.impact_summary}</p>
            </div>
          )}
        </div>
        ...
        <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
          ...
          <div className="rounded border border-slate-800 bg-slate-900 p-2.5">
            ...
            <div className={clsx(
              ...
              {result.simulated_gap > 0 ? '+' : ''}{result.simulated_gap.toLocaleString()}
            </div>
            <div className="text-2xs text-slate-600">
              {result.simulated_gap > 0 ? 'persons unallocated' : 'surplus capacity'}
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center gap-4 rounded-lg border border-slate-800 bg-slate-900 p-3">
          <div>
            <div className="text-2xs text-slate-500 mb-1">Baseline Priority</div>
            <PriorityBadge priority={h.relocation_priority.priority} size="lg" />
          </div>
          <ChevronRight className="h-4 w-4 text-slate-600" />
          <div>
            <div className="text-2xs text-slate-500 mb-1">Simulated Priority</div>
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div className="rounded-lg border border-slate-800 bg-slate-900 p-4">
            <div className="flex items-center justify-between mb-3">
              <span className="text-xs font-semibold text-slate-400">Demand vs Capacity</span>
              <DataTypeBadge type="SIMULATED" />
            </div>
            ...
            {result.simulated_gap > 0 && (
              <div className="mt-2 flex items-center gap-2 rounded border border-red-500/30 bg-red-500/8 px-2 py-1.5">
                <AlertTriangle className="h-3.5 w-3.5 text-red-400 flex-shrink-0" />
                <span className="text-2xs text-red-400">
                  {result.simulated_gap.toLocaleString()} persons cannot be allocated under this scenario
                </span>
              </div>
            )}
          </div>
          ...
          <div className="rounded-lg border border-slate-800 bg-slate-900 p-4">
            <div className="flex items-center justify-between mb-3">
              <span className="text-xs font-semibold text-slate-400">Risk: Baseline vs Simulated</span>
              <DataTypeBadge type="SIMULATED" />
            </div>
            <ResponsiveContainer width="100%" height={140}>
              <BarChart
                data={[
                  ...
                  { name: 'Simulated', value: result.simulated_risk, fill: result.simulated_risk >= 80 ? '#ef4444' : result.simulated_risk >= 60 ? '#f97316' : '#eab308' },
                ]}
                barSize={40}
              >
                <XAxis dataKey="name" tick={{ fontSize: 10, fill: '#64748b' }} axisLine={false} tickLine={false} />
                ...
                <Bar dataKey="value" radius={[3, 3, 0, 0]}>
                  {[
                    ...
                    { name: 'Simulated', fill: result.simulated_risk >= 80 ? '#ef4444' : '#f97316' },
                  ].map(entry => (
        ...
        <div className="rounded border border-slate-800 bg-slate-900/50 p-3">
          <div className="flex items-start gap-2">
            ...
            <p className="text-2xs text-slate-600 leading-relaxed">
              Scenario results are SIMULATED using the Sentinel AI risk and capacity models with the parameters above.
              They are not live forecasts. Rainfall multiplier affects the hazard component of the risk model.
              Capacity reduction applies uniformly across all sites. Road disruption removes Site B from the available pool.
              All results require human authority review before any planning decision.
            </p>
          </div>
        </div>
