...
import {
  ArrowLeft, AlertTriangle, Users, Home, MapPin,
...
function EvidenceStep({ item }: { item: typeof DEMO_MUNNAR_CENTRAL.evidence_chain[0] }) {
  return (
    <div className="flex gap-3">
      ...
      <div className="pb-4 flex-1">
        ...
        {item.source && (
          <p className="mt-1 text-2xs text-slate-600">Source: {item.source}</p>
        )}
      </div>
    </div>
  )
...
export function HabitationInvestigation() {
  ...
  // For now, always show Munnar Central (the demo case)
  // In Phase 2, fetch from API based on id
  const h = DEMO_MUNNAR_CENTRAL
  const selectedHabitation = DEMO_HABITATIONS.find(x => x.id === (id || 'munnar-central'))
    ...
    <div className="flex h-full overflow-hidden">
      ...
      <div className="flex w-80 flex-shrink-0 flex-col overflow-y-auto border-r border-slate-800 bg-slate-950">
        ...
        <div className="border-b border-slate-800 p-3">
          ...
          <div className="mt-3 grid grid-cols-3 gap-2">
            ...
            <div className="rounded border border-slate-800 bg-slate-900 p-2 text-center">
              ...
              <div className="text-lg font-bold font-mono text-red-400">+{h.risk.change}</div>
            </div>
          </div>
        ...
        <div className="border-b border-slate-800 p-3">
          ...
          <div className="flex flex-col gap-2">
            {h.hazards.map(hazard => (
              <div
                key={hazard.type}
                className={clsx('rounded border p-2.5', HAZARD_COLORS[hazard.type])}
              >
        ...
        <div className="border-b border-slate-800 p-3">
          ...
          <div>
            {h.evidence_chain.map(item => (
              <EvidenceStep key={item.step} item={item} />
            ))}
          </div>
        </div>
        ...
        <div className="border-b border-slate-800 p-3">
          ...
          <div className={clsx(
            ...
            <span className="text-xs font-semibold">
              {h.permanent_settlement_suitable ? 'POTENTIALLY SUITABLE' : 'NOT SUITABLE'}
            </span>
          </div>
      ...
      <div className="relative flex-1">
        <MapContainer
          ...
          selectedHabitationId={id || 'munnar-central'}
        ...
        <div className="absolute right-3 top-3 rounded border border-slate-700 bg-slate-900/95 p-3">
          <div className="text-2xs text-slate-500 mb-1">Munnar Central</div>
          <div className="flex items-baseline gap-2">
            <span className="text-3xl font-bold font-mono text-red-400">{h.risk.current}</span>
            <span className="text-xs text-slate-500">/100</span>
          </div>
          <div className="text-xs text-red-400 font-mono">+{h.risk.change} from baseline</div>
          <div className="mt-2">
