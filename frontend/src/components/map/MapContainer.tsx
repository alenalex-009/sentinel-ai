...
export function MapContainer({
  ...
  useEffect(() => {
    ...
    const map = new maplibregl.Map({
      ...
      // CARTO Dark Matter — keyless professional dark basemap
      style: {
        version: 8,
        sources: {
          'carto-dark': {
      ...
      map.addSource('idukki-boundary', {
        ...
        data: {
          ...
          geometry: {
            ...
            coordinates: [[
              [76.70, 9.80], [77.40, 9.80], [77.40, 10.45],
              [76.70, 10.45], [76.70, 9.80],
            ]],
          },
    ...
    ;(window as unknown as Record<string, unknown>).sentinelSelectHabitation = (id: string) => {
      handleSelect(id)
      if (popupRef.current) popupRef.current.remove()
    }
  ...
  useEffect(() => {
    ...
    if (selectedHabitationId) {
      map.setPaintProperty('habitations-circle', 'circle-stroke-width', [
        'case',
        ['==', ['get', 'id'], selectedHabitationId], 3,
        1.5,
      ])
      map.setPaintProperty('habitations-circle', 'circle-stroke-color', [
        'case',
        ['==', ['get', 'id'], selectedHabitationId], '#ffffff',
        '#0a0f1a',
      ])
  ...
  useEffect(() => {
    ...
    map.setPaintProperty('bhuvan-flood-layer', 'raster-opacity', showHazardLayer ? 0.5 : 0)
  }, [showHazardLayer])
